// app/table-status-dashboard/page.jsx (Pitch Black Mode)
"use client";
...
export default function TableStatusDashboard() {
  ...
  const loadData = async () => {
    try {
      const tableData = await fetchTableStatus();
      const rows = Array.isArray(tableData) ? tableData : (Array.isArray(tableData?.tables) ? tableData.tables : []);
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        >
          <div className="flex items-center gap-4">
            <SidebarTrigger />
            <h1 className="text-xl font-bold text-gray-800 dark:text-zinc-50">จัดการโต๊ะอาหาร</h1>
          </div>
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  disabled={isLoading}
                  className="h-9 w-9 bg-red-50 text-red-600 hover:bg-red-100 border border-red-200
                             dark:bg-red-950/30 dark:text-red-500 dark:hover:bg-red-900/50 dark:border-red-900/50"
                >
        ...
        {/* Main Background: ดำสนิท (dark:bg-black) */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {tables.map((table) => {
              ...
              return (
                <Card
                  key={table.table_id}
                  // Card Background: ดำสนิทและขอบเข้ม (dark:bg-black dark:border-zinc-900)
                  className={`flex flex-col overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-t-4 ${getBorderColor(status)} bg-white relative dark:bg-black dark:border-zinc-900 dark:shadow-none`}
                >
                  {isMerged && (
                    <div className="absolute top-2 right-2 z-10">
                      <Badge
                        variant="secondary"
                        className="bg-blue-50 text-blue-600 border-blue-100 gap-1 shadow-none dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-900/50"
                      >
                  ...
                  <CardHeader
                    className="pb-3 flex flex-row items-center justify-between space-y-0 border-b border-gray-50 dark:border-zinc-900"
                  >
                  ...
                  <CardContent className="flex-1 py-4">
                    {status === "ว่าง" && (
                      <div
                        className="h-24 flex flex-col items-center justify-center text-emerald-400 border-2 border-dashed border-emerald-50 rounded-lg bg-emerald-50/20
                                   dark:text-emerald-500 dark:border-emerald-950 dark:bg-emerald-950/10"
                      >
                    ...
                    {status !== "ว่าง" && (
                      <div
                        className={`h-24 flex flex-col items-center justify-center border-2 border-dashed rounded-lg ${status === "รอชำระ"
                            ? "text-red-400 border-red-100 bg-red-50/20 dark:text-red-500 dark:border-red-950 dark:bg-red-950/10"
                            : "text-orange-400 border-orange-100 bg-orange-50/20 dark:text-orange-500 dark:border-orange-950 dark:bg-orange-950/10"
                          }`}
                      >
                        {isMerged && linkedTables.length > 0 ? (
                          <div className="text-center px-1 w-full max-h-20 overflow-y-auto custom-scrollbar">
                            <div
                              className="flex items-center justify-center gap-1 text-blue-600 mb-1.5 sticky top-0 bg-opacity-90 backdrop-blur-[1px] dark:text-blue-400 dark:bg-black"
                            >
                            ...
                            <div className="flex flex-wrap justify-center gap-1.5 px-1">
                              {linkedTables.map(t => (
                                <Badge
                                  key={t.table_id}
                                  variant="outline"
                                  className="bg-white text-gray-700 text-[10px] h-5 px-1.5 border-blue-300 shadow-sm
                                             dark:bg-zinc-900 dark:text-zinc-300 dark:border-blue-900"
                                >
                                  {t.number}
                                </Badge>
                              ))}
                  ...
                  <CardFooter
                    className="pt-0 flex flex-col gap-2 p-4 bg-gray-50/30 dark:bg-zinc-900/20"
                  >
                    {status === "ว่าง" && (
                      <Button
                        className="w-full bg-gray-900 hover:bg-gray-800 text-white dark:bg-zinc-100 dark:hover:bg-zinc-200 dark:text-black"
                        onClick={() => handleAction(table.table_id, "startOrder")}
                      >
                    ...
                    {status === "มีลูกค้า" && (
                      <div className="space-y-2 w-full">
                        <div className="grid grid-cols-2 gap-2">
                          <Button
                            variant="outline"
                            onClick={() => router.push(`/orders?table_id=${table.table_id}`)}
                            className="dark:border-zinc-800 dark:text-blue-400 dark:hover:bg-zinc-900"
                          >
                            <PlusCircle className="mr-2 h-4 w-4 text-blue-600 dark:text-blue-400" /> สั่งเพิ่ม
                          </Button>
                          <Button
                            onClick={() => router.push(`/billing?table_id=${table.table_id}`)}
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <Button
                            ...
                            className="text-xs h-8 bg-gray-200 hover:bg-gray-300 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-50"
                          >
                          ...
                          <Button
                            ...
                            className="text-xs h-8 bg-blue-100 hover:bg-blue-200 text-blue-700 border border-blue-200
                                       dark:bg-blue-900/30 dark:hover:bg-blue-800/50 dark:text-blue-300 dark:border-blue-900/50"
                          >
                        ...
                        {isMerged && (
                          <Button
                            ...
                            className="w-full h-8 text-xs bg-red-100 hover:bg-red-200 text-red-700 border border-red-200 shadow-none mt-1
                                       dark:bg-red-900/30 dark:hover:bg-red-800/50 dark:text-red-300 dark:border-red-900/50"
                          >
                            <Unlink className="mr-1 h-3 w-3" /> แยกโต๊ะออกจากกลุ่ม
                          </Button>
                        )}
                      </div>
                    )}
                    ...
                    {status === "รอชำระ" && (
                      <Button
                        variant="destructive"
                        onClick={() => handleAction(table.table_id, "changeStatus", "ว่าง")}
                        className="w-full"
                      >
      ...
      {/* --- Modals (ปรับพื้นหลังเป็นดำสนิท) --- */}
      <Dialog open={isMoveOpen} onOpenChange={setIsMoveOpen}>
        <DialogContent className="dark:bg-black dark:border-zinc-900">
          <DialogHeader>
            <DialogTitle className="dark:text-zinc-50">ย้ายโต๊ะ (Move Table)</DialogTitle>
            <DialogDescription className="dark:text-zinc-400">
              จาก <b className="dark:text-zinc-50">โต๊ะ {selectedTable?.number}</b> ไปยังโต๊ะใหม่
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Label className="dark:text-zinc-300">เลือกโต๊ะปลายทาง (ว่าง)</Label>
            <select
              className="flex h-10 w-full rounded-md border px-3 py-2 text-sm dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-50 outline-none"
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmMove} disabled={!targetTableId || isLoading}>ยืนยัน</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isMergeOpen} onOpenChange={setIsMergeOpen}>
        <DialogContent className="dark:bg-black dark:border-zinc-900">
          <DialogHeader>
            <DialogTitle className="dark:text-zinc-50">รวมโต๊ะ (Merge Table)</DialogTitle>
            <DialogDescription className="dark:text-zinc-400">
          ...
          <DialogFooter>
            <Button onClick={handleConfirmMerge} disabled={!mergeTargetId || isLoading}>ยืนยัน</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </SidebarProvider>
