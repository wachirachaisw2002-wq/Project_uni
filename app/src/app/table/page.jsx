...
export default function TableStatusDashboard() {
  ...
  // ✅ State สำหรับชื่อและเบอร์โทร (Takeout)
  ...
  const loadData = async () => {
    try {
      ...
      // รองรับโครงสร้างข้อมูลทั้งแบบใหม่และเก่า
      if (data.tables) {
  ...
  const handleOpenTakeoutModal = () => {
    setTakeoutName("");
    setTakeoutPhone(""); // ✅ รีเซ็ตเบอร์โทรทุกครั้งที่เปิด Modal
  ...
  const handleConfirmTakeout = () => {
    ...
    // ✅ ส่งทั้งชื่อและเบอร์โทรไปทาง URL Params
  ...
  const handleExistingTakeout = (takeout) => {
    const params = new URLSearchParams({
      ...
      customerPhone: takeout.customer_phone || "" // ส่งเบอร์เดิมไปด้วย (ถ้ามี)
    });
    router.push(`/orders?${params.toString()}`);
  };
  ...
  // --- Functions จัดการโต๊ะ (เหมือนเดิม) ---
      ...
      <SidebarInset>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="destructive" size="icon" onClick={handleResetAllTables} disabled={isLoading} className="h-9 w-9 bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 dark:bg-red-950/30 dark:text-red-500 dark:hover:bg-red-900/50 dark:border-red-900/50">
        ...
        <main className="p-6 bg-gray-50/50 min-h-[calc(100vh-4rem)] dark:bg-black">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            {/* === 1. ปุ่มสร้างออเดอร์กลับบ้านใหม่ === */}
            <Card
            ...
            {/* === 2. รายการสั่งกลับบ้านที่ค้างอยู่ (แสดงเบอร์โทรด้วย) === */}
        ...
        <Dialog open={isTakeoutOpen} onOpenChange={setIsTakeoutOpen}>
          <DialogContent className="dark:bg-black dark:border-zinc-900">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleConfirmTakeout} disabled={!takeoutName.trim()} className="bg-purple-600 hover:bg-purple-700 dark:bg-purple-700 dark:hover:bg-purple-600 text-white">
                ยืนยันเปิดบิล
              </Button>
            </DialogFooter>
          </DialogContent>
