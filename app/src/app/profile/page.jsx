...
export default function ProfilePage() {
  ...
  // 1. formData สำหรับเก็บค่าที่กำลังพิมพ์แก้ไข (Editing)
  ...
  // 2. displayData สำหรับเก็บค่าจริงเพื่อแสดงผล (Display Only)
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // เซตค่าเริ่มต้นให้ทั้งคู่เหมือนกัน
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // เมื่อบันทึกสำเร็จ อัปเดต displayData ให้ตรงกับข้อมูลใหม่
        setDisplayData(formData); 
        
        setTimeout(() => {
          alert("บันทึกข้อมูลเรียบร้อยแล้ว");
          // ไม่ต้อง reload แล้วเพื่อให้ UX ลื่นไหล (หรือจะ reload ก็ได้)
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset className="bg-zinc-50/50 dark:bg-black">
        ...
        {/* ✅ แก้ไข Header ตรงนี้: เปลี่ยนเป็น bg-white, เพิ่ม z-50, ลบ blur */}
